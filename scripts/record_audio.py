...
def create_lsl_stream(stream_name, stream_type, channels, rate, stream_id):
    """
    Creates and returns an LSL stream outlet based on the provided parameters using pylsl.
    """
    info = StreamInfo(name=stream_name, type=stream_type, channel_count=channels,
                      nominal_srate=rate, channel_format='int16', source_id=stream_id)
...
def audio_stream_callback(outlet, in_data, frame_count, time_info, status):
    """
    Callback function to stream audio data into LSL using pylsl.
    Converts audio data to a numpy array and pushes it into the LSL stream via pylsl.
    """
