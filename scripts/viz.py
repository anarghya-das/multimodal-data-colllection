...
class DataInlet(Inlet):
    ...
    def __init__(self, info: pylsl.StreamInfo, plt: pg.PlotItem):
        ...
        bufsize = (2 * math.ceil(info.nominal_srate() *
                   plot_duration), info.channel_count())
        ...
        self.curves = [pg.PlotCurveItem(
            x=empty, y=empty, autoDownsample=True) for _ in range(self.channel_count)]
    ...
    def pull_and_plot(self, plot_time, plt):
        ...
        if ts:
            ...
            for ch_ix in range(self.channel_count):
                ...
                this_y = np.hstack(
                    (old_y[old_offset:], y[new_offset:, ch_ix] - ch_ix))
